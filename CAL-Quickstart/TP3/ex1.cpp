...
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    for (int i = 0; i < vp.size(); i++) {
        for (int j = 0; j < vp.size(); j++) {
            ...
            if (vp[i].distance(vp[j]) < minDist) {
                minDist = vp[i].distance(vp[j]);
                res = Result(minDist, vp[i], vp[j]);
            }
        }
    }
    return res;
...
// d)
// Invariante do ciclo principal [ vp(i) ]
// Variante do ciclo principal [ i++ ]: estritamente decrescente
...
Result nearestPoints_DC_lr(std::vector<Point> &vp, int left, int right) {
    // Invalid
    if (left >= right)
        return Result();
    ...
    // Divide x in left and right
    int middle = (right+left)/2;
    Result rl = nearestPoints_DC_lr(vp, left, middle), rr = nearestPoints_DC_lr(vp, middle+1, right);

    // Find delta (min(leftPart, rightPart))
    Result res;
    if (rl.dmin < rr.dmin) {
        res = rl;
    }
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    return nearestPoints_DC_lr(vp, 0, vp.size()-1);
}


Result nearestPoints_DC_MT_lr(std::vector<Point> &vp, int left, int right) {
    // Invalid
    if (left >= right)
        return Result();
    ...
    Result rl, rr;
    if (numThreads > 1) {
        std::thread t([&rl, &vp, left, middle](){rl = nearestPoints_DC_MT_lr(vp, left, middle);});
        Result rr = nearestPoints_DC_MT_lr(vp, middle+1, right);
        t.join();
    }
    else {
        rl = nearestPoints_DC_lr(vp, left, middle), rr = nearestPoints_DC_lr(vp, middle+1, right);
    }
